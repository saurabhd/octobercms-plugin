<?php namespace SaurabhDhariwal\Revisionhistory\Classes;
/*
class.Diff.php
...
Extended by Alexandre Couedelo to the project Wiki-plugin for octoberCMS
*/
...
class Diff{
  ...
  /* Returns a diff as a string, with only insertions and deletions plus line
   * number deletions are prefixed by '- ', and insertions are prefixed
   ...
   *              to "\n"
   */
  public static function toResidualString($diff, $separator = "\n"){
    // initialise the string
    $string = '';
    $i=1;
    $prev=self::UNMODIFIED;
    $sequence=1;
    // loop over the lines in the diff
    foreach ($diff as $line){
      // extend the string with the line
      switch ($line[1]){
        ...
        case self::INSERTED :
         //sequence treatment
         if($prev == self::DELETED && $sequence>1) {
           $i = $sequence + 1;
           $sequence = 1;
         }
         //line
